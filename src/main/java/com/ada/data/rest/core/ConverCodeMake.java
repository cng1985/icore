...
public class ConverCodeMake {
	...
	public static void main(String[] args) {
		...
		make.make2(AppVersion.class, App.class);
	...
	private static void x() {
		BeanInfo info;
		try {
			info = Introspector.getBeanInfo(App.class);
			PropertyDescriptor[] descritors = info.getPropertyDescriptors();// 获得
																			// beans
																			// PropertyDescriptor。
			...
			for (int index = 0; index < size; index++) {
				...
				// descritors[index].getReadMethod().
			}
		} catch (IntrospectionException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
